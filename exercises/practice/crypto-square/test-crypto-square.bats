...
@test "Lowercase" {
    [[ $BATS_RUN_SKIPPED == "true" ]] || skip
    run gawk -f crypto-square.awk <<< "A"
    assert_success
    assert_output "a"
}
...
@test "Remove spaces" {
    [[ $BATS_RUN_SKIPPED == "true" ]] || skip
    run gawk -f crypto-square.awk <<< "  b "
    assert_success
    assert_output "b"
}
...
@test "Remove punctuation" {
    [[ $BATS_RUN_SKIPPED == "true" ]] || skip
    run gawk -f crypto-square.awk <<< "@1,%!"
    assert_success
    assert_output "1"
}
...
@test "9 character plaintext results in 3 chunks of 3 characters" {
    [[ $BATS_RUN_SKIPPED == "true" ]] || skip
    run gawk -f crypto-square.awk <<< "This is fun!"
    assert_success
...
@test "8 character plaintext results in 3 chunks, the last one with a trailing space" {
    [[ $BATS_RUN_SKIPPED == "true" ]] || skip
    run gawk -f crypto-square.awk <<< "Chill out."
    assert_success
